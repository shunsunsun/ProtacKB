...
def createNodes(tx):
    ...
    ###Extend protein nodes on Reactome, GOBP and GOMF
    ...
    #working with iGLUE and Warhead compounds from OpenTargets and update nodes with OT links

    advEve = {'AdvEffect':{}}
    print('creating nodes for openTar adverse effects')

    infile = open('data/iglue_warhead_openTarget.pkl', 'rb')
    iglue_warhead_openTarget = pickle.load(infile)
    infile.close()

    iglue_ot = iglue_warhead_openTarget['iglue']
    warhead_ot = iglue_warhead_openTarget['warhead']

    for key in iglue_ot.keys():
        for item in iglue_ot[key]['drug']['adverseEvents']['rows']:

            if item['name'] in advEve['AdvEffect']:
                continue

            advEve['AdvEffect'][item['name']] = Node('AdvEffect', ** {'Name':item['name'],
                                                                      'MeddraCode':f"http://purl.bioontology.org/ontology/MEDDRA/{item['meddraCode']}"})


    for key in warhead_ot.keys():
        for item in warhead_ot[key]['drug']['adverseEvents']['rows']:
    ...
    node_dict.update(advEve)

    print('Adv Eve nodes created')

    print('Opentargets Pickle file read')
    print('working with iGLUE')

    for chem in iglue_ot.keys():
        for item in node_dict['iGLUE'].keys():
            if 'ChEMBL' in node_dict['iGLUE'][item]:
                # print(node_dict['iGLUE'][item]['ChEMBL'])
                if chem == node_dict['iGLUE'][item]['ChEMBL'].split('/')[-1]:
                    # print(chem)
                    # print(node_dict['iGLUE'][item]['ChEMBL'].split('/')[-1])
                    node_dict["iGLUE"][item].update({'OpenTarget': f"https://platform.opentargets.org/drug/{chem}"})
                    #print(node_dict["iGLUE"][item])
                    # d.update({'OpenTarget':f"https://platform.opentargets.org/drug/{chem}"})
                    # for adv in iglue_ot[chem]['drug']['adverseEvents']['rows']:
                    #     chem2adveff = Relationship(node_dict['iGLUE'][item], 'hasAdverseEffect',
                    #                                node_dict['AdvEffect'][adv['name']])
                    #     tx.create(chem2adveff)

    print('working with warhead')
    for chem in warhead_ot.keys():
        for item in node_dict['Warhead'].keys():
            if 'ChEMBL' in node_dict['Warhead'][item]:
                # print(node_dict['iGLUE'][item]['ChEMBL'])
                if chem == node_dict['Warhead'][item]['ChEMBL'].split('/')[-1]:
                    # print(chem)
                    # print(node_dict['iGLUE'][item]['ChEMBL'].split('/')[-1])
                    node_dict["Warhead"][item].update({'OpenTarget': f"https://platform.opentargets.org/drug/{chem}"})
                    # d.update({'OpenTarget':f"https://platform.opentargets.org/drug/{chem}"})
                    # for adv in warhead_ot[chem]['drug']['adverseEvents']['rows']:
                    #     chem2adveff = Relationship(node_dict['Warhead'][item], 'hasAdverseEffect',
                    #                                node_dict['AdvEffect'][adv['name']])
                    #     tx.create(chem2adveff)

    #update protein nodes with opentarget links
    for prot in node_dict["Protein"].keys():
        if node_dict['Protein'][prot]['Uniprot']:
            if node_dict['Protein'][prot]['Uniprot'] != 'nan':
                id = str(node_dict["Protein"][prot]['Uniprot'])
                ret_uprot = requests.get('https://www.uniprot.org/uniprot/' + id + '.txt').text.split('\n')
                for line in ret_uprot:
                    if 'OpenTargets' in line:
                        ensg = line.split(";")[1]
                        ensg = ensg.split(' ')[1]
                        node_dict["Protein"][prot].update({'OpenTargets':f"https://platform.opentargets.org/target/{ensg}"})

    #Add iglue and warhead associated mech of action and diseases as nodes

    infile = open('data/iglue_warhead_chembl_mechDis.pkl', 'rb')
    iglue_warhead_chembl = pickle.load(infile)
    infile.close()

    iglue_mech = iglue_warhead_chembl['iglue_mech']
    iglue_dis = iglue_warhead_chembl['iglue_dis']
    warhead_mech = iglue_warhead_chembl['warhead_mech']
    warhead_dis = iglue_warhead_chembl['warhead_dis']

    mech = {'MOA':{}}

    print('adding mech nodes')

    for item in tqdm(iglue_mech.keys(), total = len(iglue_mech.keys()), desc ='adding MOA nodes'):
        for i in range(len(iglue_mech[item])):

            if iglue_mech[item][i]['mechanism_of_action'] in mech['MOA']:
                continue

            mech['MOA'][iglue_mech[item][i]['mechanism_of_action']] = Node('MOA', ** {'Name':iglue_mech[item][i]['mechanism_of_action']})

    for item in warhead_mech.keys():
        for i in range(len(warhead_mech[item])):

            if warhead_mech[item][i]['mechanism_of_action'] in mech['MOA']:
                continue

            mech['MOA'][warhead_mech[item][i]['mechanism_of_action']] = Node('MOA', ** {'Name': warhead_mech[item][i]['mechanism_of_action']})

    node_dict.update(mech)

    print('adding dis nodes')
    for item in iglue_dis.keys():
        for i in range(len(iglue_dis[item])):

            if iglue_dis[item][i]['mesh_heading'] in node_dict['Disease']:
                continue

            node_dict['Disease'][iglue_dis[item][i]['mesh_heading']] = Node('Disease', ** {'Name':iglue_dis[item][i]['mesh_heading']})

    for item in warhead_dis.keys():
        for i in range(len(warhead_dis[item])):

            if warhead_dis[item][i]['mesh_heading'] in node_dict['Disease']:
                continue

            node_dict['Disease'][warhead_dis[item][i]['mesh_heading']] = Node('Disease', ** {'Name':warhead_dis[item][i]['mesh_heading']})
...
#comment following codes to be faster
    ...
    #print(node_dict['Disease'])
    return(node_dict)

def prot2opentar(tx,ptacNode):
    import requests

    print('updating proteins with opentar links')
    for prot in ptacNode["Protein"].keys():
        if ptacNode['Protein'][prot]['Uniprot']:
            if ptacNode['Protein'][prot]['Uniprot'] != 'nan':
                id = str(ptacNode["Protein"][prot]['Uniprot'])
                ret_uprot = requests.get('https://www.uniprot.org/uniprot/' + id + '.txt').text.split('\n')
                for line in ret_uprot:
                    if 'OpenTargets' in line:
                        ensg = line.split(";")[1]
                        ensg = ensg.split(' ')[1]
                        ptacNode["Protein"][prot].update({'OpenTargets':f"https://platform.opentargets.org/target/{ensg}"})
                        print(ptacNode["Protein"][prot])

    # comment following codes to be faster
    # Add also updated nodes into graph
    print('updating nodes after updating prot nodes with ensg')
    for node_type in ptacNode:
        _add_nodes(
            node_dict=ptacNode[node_type],
            tx=tx
        )

    return (ptacNode)

def createReln_iglue_whead2mechdis(tx,ptacNode):
    ...
    print('creating iglue/warhead to disease relations')

    for chem in tqdm(iglue_dis.keys(),total=len(iglue_dis.keys()),desc='create rel iglue to dis'):
        for item in ptacNode['iGLUE'].keys():
            if 'ChEMBL' in ptacNode['iGLUE'][item]:
                # print(node_dict['iGLUE'][item]['ChEMBL'])
                if chem == ptacNode['iGLUE'][item]['ChEMBL'].split('/')[-1]:
                    for i in range(len(iglue_dis[chem])):
                        chem2dis = Relationship(ptacNode['iGLUE'][item], 'usedAgainst', ptacNode['Disease'][iglue_dis[chem][i]['mesh_heading']])
                        tx.create(chem2dis)

    for chem in warhead_dis.keys():
        for item in ptacNode['Warhead'].keys():
            if 'ChEMBL' in ptacNode['Warhead'][item]:
                # print(node_dict['iGLUE'][item]['ChEMBL'])
                if chem == ptacNode['Warhead'][item]['ChEMBL'].split('/')[-1]:
                    for i in range(len(warhead_dis[chem])):
                        chem2dis = Relationship(ptacNode['Warhead'][item], 'usedAgainst', ptacNode['Disease'][warhead_dis[chem][i]['mesh_heading']])
                        tx.create(chem2dis)

    print('creating iglue/whead to mech relations')

    for chem in iglue_mech.keys():
        for item in ptacNode['iGLUE'].keys():
            if 'ChEMBL' in ptacNode['iGLUE'][item]:
                # print(node_dict['iGLUE'][item]['ChEMBL'])
                if chem == ptacNode['iGLUE'][item]['ChEMBL'].split('/')[-1]:
                    for i in range(len(iglue_mech[chem])):
                        chem2mech = Relationship(ptacNode['iGLUE'][item], 'hasMechOfAction', ptacNode['MOA'][iglue_mech[chem][i]['mechanism_of_action']])
                        tx.create(chem2mech)

    for chem in warhead_mech.keys():
        for item in ptacNode['Warhead'].keys():
            if 'ChEMBL' in ptacNode['Warhead'][item]:
                # print(node_dict['iGLUE'][item]['ChEMBL'])
                if chem == ptacNode['Warhead'][item]['ChEMBL'].split('/')[-1]:
                    for i in range(len(warhead_mech[chem])):
                        chem2mech = Relationship(ptacNode['Warhead'][item], 'hasMechOfAction', ptacNode['MOA'][warhead_mech[chem][i]['mechanism_of_action']])
                        tx.create(chem2mech)

def createReln_openTar(tx,ptacNode):
        ...
        for item in ptacNode['iGLUE'].keys():
            if 'ChEMBL' in ptacNode['iGLUE'][item]:
                ...
                if chem == ptacNode['iGLUE'][item]['ChEMBL'].split('/')[-1]:
                    # print(chem)
                    # print(node_dict['iGLUE'][item]['ChEMBL'].split('/')[-1])
                    #ptacNode["iGLUE"][item].update({'OpenTarget': f"https://platform.opentargets.org/drug/{chem}"})
                    #print(ptacNode["iGLUE"][item])
                    # d.update({'OpenTarget':f"https://platform.opentargets.org/drug/{chem}"})
                    for adv in iglue_ot[chem]['drug']['adverseEvents']['rows']:
                        chem2adveff = Relationship(ptacNode['iGLUE'][item], 'hasAdverseEffect',
                                                   ptacNode['AdvEffect'][adv['name']])
                        tx.create(chem2adveff)
        ...
        for item in ptacNode['Warhead'].keys():
            if 'ChEMBL' in ptacNode['Warhead'][item]:
                ...
                if chem == ptacNode['Warhead'][item]['ChEMBL'].split('/')[-1]:
                    # print(chem)
                    # print(node_dict['iGLUE'][item]['ChEMBL'].split('/')[-1])
                    #ptacNode["Warhead"][item].update({'OpenTarget': f"https://platform.opentargets.org/drug/{chem}"})
                    # d.update({'OpenTarget':f"https://platform.opentargets.org/drug/{chem}"})
                    for adv in warhead_ot[chem]['drug']['adverseEvents']['rows']:
                        chem2adveff = Relationship(ptacNode['Warhead'][item], 'hasAdverseEffect',
                                                   ptacNode['AdvEffect'][adv['name']])
                        tx.create(chem2adveff)

    print('finished working with iGLUE and Warhead, Relationships created')
...
def createReln_uniprot(tx,ptacNode):
    ...
    for p in tqdm(ptacNode['Protein'],total=len(ptacNode['Protein']),desc='working with uprotids'):
        #print('start')
...
def createGraph():
    ...
    #db_name, graph = populate_db("ptac")

    # ADMIN_NAME = "itmp"
    # ADMIN_PASS = "1234"
    # URL = "bolt://localhost:7687"
    # graph = Graph(URL, auth=(ADMIN_NAME, ADMIN_PASS), name='ptac')
